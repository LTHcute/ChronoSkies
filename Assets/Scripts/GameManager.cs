...
public class GameManager : MonoBehaviour
{
	...
	private void Start()
	{
        listBackground = Resources.LoadAll<Sprite>("Background");
        Debug.Log(listBackground.Length);
    ...
    private void DrawCurvedLine(Vector2 startPos, Vector2 endPos)
    {
        ...
        if (!controlPointGenerated)
        {
            ...
            Vector2 controlOffset = cachedControlPoint - midPoint;

            if (controlOffset.x > 0)
            {
                player.GetComponent<SpriteRenderer>().flipX = false;
                Debug.Log("➡️ Đường cong lệch phải (theo màn hình)");
            }    
                
            else
            {
                player.GetComponent<SpriteRenderer>().flipX = true;
                Debug.Log("⬅️ Đường cong lệch trái (theo màn hình)");
            }
	...
	{
        background.sprite = listBackground[Random.Range(0, listBackground.Length)];
        Debug.Log("CreateNextObstacle");
        obstacleId++;
        Sprite[] imageSprite = Resources.LoadAll<Sprite>("ObstacleImage");
        Debug.Log(imageSprite.Length);
        int i = Random.Range(0, imageSprite.Length);
        Sprite image = imageSprite[i];
        tempColor = colorTable[Random.Range(0, colorTable.Length)];
        ...
        previousObstacle.GetComponent<SpriteRenderer>().sprite = image;
